...
impl<'a, T> std::ops::Mul<T> for &'a Hepta4202468<T>
where
    T: std::ops::MulAssign + Copy,
{
    type Output = Hepta4202468<T>;
...
/// Addition : &Self + &Self
impl<'a, 'b, T> Add<&'b Hepta4202468<T>> for &'a Hepta4202468<T>
where
    T: std::ops::AddAssign + Copy,
{
    type Output = Hepta4202468<T>;

    fn add(self, other: &'b Hepta4202468<T>) -> Self::Output {
        ...
        for (x, y) in new.u6.iter_mut().zip(other.u6.iter()) {
            *x += *y;
        }
        for (x, y) in new.u8.iter_mut().zip(other.u8.iter()) {
            *x += *y;
        }
        new
    }
